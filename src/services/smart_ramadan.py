...
This service provides intelligent Ramadan/Eid period detection and reference 
period selection for budget estimation.
...
class SmartRamadanSystem:
    """
    Intelligent system that adapts to any Ramadan configuration without hardcoding
    """
    ...
    def get_reference_period_for_day(self, by_month: int, by_day: int) -> Dict[str, any]:
        ...
        Returns:
            Dictionary with:
                ...
                - source_months: List of months to pull data from
                - source_day_type: 'ramadan', 'eid', or 'normal'
                - source_date_range: Tuple of (start_date, end_date) for filtering
        ...
        # RULE 2: Ramadan days use WEEKDAY AVERAGE from CY Ramadan period
            ...
            result['source_date_range'] = (self.ramadan_start_CY, self.ramadan_end_CY)
        
        # RULE 3: Normal days use WEEKDAY AVERAGE from CY normal days in same/nearby month
        else:
            ...
            result['method'] = 'weekday_average'
            result['source_period'] = f"CY {calendar.month_name[candidate_months[0]]} (Normal days)"
            result['source_months'] = candidate_months
            result['source_day_type'] = 'normal'
            # For normal days, we want the entire month excluding Ramadan/Eid
            result['source_date_range'] = None  # Will use entire month
    ...
    def generate_estimation_plan(self) -> Dict[int, Dict[int, dict]]:
        ...
        for month in affected_months:
            ...
            print(f"\n📅 {calendar.month_name[month]} {self.budget_year}:")
